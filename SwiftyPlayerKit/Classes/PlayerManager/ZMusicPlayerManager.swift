...
/// 播放状态枚举
public enum ZEnumMusicPlayerState: Int {
    /// 准备播放
    case Preparing = 1
    /// 开始播放
    case Beigin = 2
...
/// 播放各种状态代理
public protocol ZMusicPlayerDelegate: class {
    /// 更新进度
...
/// 播放状态通知Key
public let ZMusicPlayerStateChange = "ZMusicPlayerStateChange"
/// 播放地址通知Key
public let ZMusicPlayerCurrentPlayUrl = "ZMusicPlayerCurrentPlayUrl"
/// 播放类型通知Key
public let ZMusicPlayerPlayType = "ZMusicPlayerPlayType"
/// 播放方式
public enum ZMusicPlayerType: Int {
    /// 单曲播放 - 不循环
    case single
    /// 列表播放 - 不循环
    case list
...
/// 播放管理器
public class ZMusicPlayerManager: NSObject {
    /// 单例模式
    public static let shared = ZMusicPlayerManager()
    /// 播放器
    ...
    /// 当前播放方式
    public var playType: ZMusicPlayerType = .cycle
    /// 播放速度 改变播放速度
    ...
    /// 播放代理
    public weak var delegate: ZMusicPlayerDelegate?
    /// 时间监听器
...
extension ZMusicPlayerManager {
    /// 回复默认播放速度
    public final func resetPlaySeed() {
        self.playSpeed = 1.0
    }
    /// 设置播放速率
    public final func setPlaySpeed(playSpeed: Float) {
        if self.isPlay {
    ...
    ///   - enable:
    ///   - playerItem: 当前播放
    public final func enableAudioTracks(enable: Bool, playerItem: AVPlayerItem) {
    ...
    /// 如果需要下载重载该方法
    open func reloadAudio(playpath: String) -> URL {
    ...
    public final func playMusic(model: ZModelMusic, type: ZMusicPlayerType) {
        // 播放前初始化倍速 1.0
    ...
    public final func musicSeekToTime(time: Float) {
    ...
    /// 设置锁屏时 播放中心的播放信息
    public final func setNowPlayingInfo() {
    ...
    private func updateCurrentPlayState(state: ZEnumMusicPlayerState) {
        NotificationCenter.default.post(name: NSNotification.Name(rawValue: ZMusicPlayerStateChange),
                                        object: nil,
                                        userInfo: [ZMusicPlayerStateChange: state,
                                                   ZMusicPlayerCurrentPlayUrl: self.currentUrl ?? "",
                                                   ZMusicPlayerPlayType: self.playType])
    }
    /// 格式化时间为字符串
...
extension TimeInterval {
    fileprivate var durationText: String {
        if self.isNaN || self.isInfinite { return "00:00" }
